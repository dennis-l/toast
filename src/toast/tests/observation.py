...
class ObservationTest(MPITestCase):
    ...
    def test_partial_redist(self):
        ...
        for ob in data.obs:
            ...
            for idet, det in enumerate(ob.all_detectors):
                if det in ob.local_detectors:
                    print(
                        f"DBG {ob.name} inplace {inplace.comm.world_rank} : {det}[{ob.local_index_offset}:{ob.local_index_offset + ob.n_local_samples}] += {10*idet}"
                    )
        ...
        for ob in data.obs:
            ...
            print(f"proc_rows = {proc_rows}, gsize = {ob.comm.group_size}")
            ...
            for idet, det in enumerate(temp_ob.local_detectors):
                print(
                    f"DBG {temp_ob.name} redist {temp_ob.comm.world_rank} : {det}[{temp_ob.local_index_offset}:{temp_ob.local_index_offset + temp_ob.n_local_samples}] += {10*idet}"
                )
