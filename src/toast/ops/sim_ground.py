...
class SimGround(Operator):
    ...
    def _obs_telescopes(self, data, det_ranks, detectors):
        ...
        if detectors is None:
            ...
            keep = [
                True if x in dcheck else False for x in session_fp.detector_data["name"]
            ]
            fp_input = QTable(session_fp.detector_data[keep])

        session_keys = np.unique(fp_input[self.session_split_key])

        obs_tele = dict()
        for key in session_keys:
            fp_rows = fp_input[self.session_split_key] == key
